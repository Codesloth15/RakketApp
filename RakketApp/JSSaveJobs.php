        ...
        <div class="create-prof">
            <div class="job-container">
                <div class="job-list-container">
                    ...
                    $saveJobQuery = "SELECT * FROM save_job";
                    $saveJobResult = mysqli_query($conn, $saveJobQuery);

                    while ($savedJobRow = mysqli_fetch_array($saveJobResult)) {
                        if ($savedJobRow['EMAIL'] == $email) {
                            $jobId = $savedJobRow['USERID'];
                            $jobInfoQuery = "SELECT * FROM job_info WHERE id = $jobId";
                            $jobInfoResult = mysqli_query($conn, $jobInfoQuery);

                            while ($jobInfoRow = mysqli_fetch_array($jobInfoResult)) {
                            
                                echo "<br>
            <form class='job-card' action='JobseekerHome.php' method='post'>
                <input type='hidden' name='id' value='{$jobInfoRow['id']}'>
                <h3 id=''>{$jobInfoRow['Role']}</h3>
                <h4 id=''>{$jobInfoRow['EmployerNAme']}</h4>
                <h5 id='' >{$jobInfoRow['Address']}</h5>
                <p  id=''>{$jobInfoRow['Job_desc']}</p>
                <input type='submit'class='btn_1'  value='Remove' name='delete'>
